...
export function SwanHistoricalAnalysis() {
  const { accountType } = useImprovedAuth();
  const { data, isLoading, error } = useHistoricalAnalysis();
  const { openPricingModal } = usePricingModal();
  const [selectedAnalysis, setSelectedAnalysis] =
    useState<HistoricalAnalysisData | null>(null);
  ...
  const handleAnalysisClick = (analysis: HistoricalAnalysisData) => {
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm text-zinc-400 mb-6 max-w-md">
            Access past market signals and analysis to understand market trends
            over time.
          </p>
          <Button
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    ...
                    >
                      Score: {Math.round(analysis.blackSwanScore)}
                    </div>
                  </div>
                </div>
              </div>
